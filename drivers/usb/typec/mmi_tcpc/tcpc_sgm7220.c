...
static void sgm7220_process_mode_register(struct sgm7220_chip *chip, u8 status)
{
	...
	chip->type_c_param.active_cable_attach = val;

	pr_info("sgm7220 %s , current_det = %d, accessory_attach = %d, active_cable_attach = %d\n",
		__func__,
		chip->type_c_param.current_det,
		chip->type_c_param.accessory_attach,
		chip->type_c_param.active_cable_attach);
}
...
static void sgm7220_process_interrupt_register(struct sgm7220_chip *chip, u8 status)
{
	...
	if (chip->type_c_param.attach_state == CABLE_STATE_AS_DFP ||
		...
		val = (HOST_CUR_1P5 << MOD_CURRENT_MODE_ADVERTISE_SHIFT);
	}
	...
	case CABLE_STATE_TO_ACCESSORY:
		if (chip->type_c_param.accessory_attach == ACCESSORY_AUDIO
...
static int sgm_parse_dt(struct sgm7220_chip *chip, struct device *dev)
{
	...
	ret = of_property_read_u32(
		np, "sgm7220,intr_gpio_num", &chip->irq_gpio);
	if (ret < 0)
		pr_err("%s no intr_gpio info\n", __func__);
#endif
	return ret;
...
static int sgm7220_initialization(struct sgm7220_chip *chip)
{
	...
	reg_val = (HOST_CUR_1P5 << MOD_CURRENT_MODE_ADVERTISE_SHIFT);
